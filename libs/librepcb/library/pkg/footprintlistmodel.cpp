...
void FootprintListModel::setFootprintList(FootprintList* list) noexcept {
  ...
  if (mFootprintList) {
    // for (Footprint& item : *mFootprintList) {
    //  disconnect(&item, &Footprint::edited, this,
    //             &FootprintListModel::signalEdited);
    //}
    mFootprintList->unregisterObserver(this);
  }
  ...
  if (mFootprintList) {
    ...
    // for (Footprint& item : *mFootprintList) {
    //  connect(&item, &Footprint::edited, this,
    //          &FootprintListModel::signalEdited);
    //}
  }
...
void FootprintListModel::listObjectAdded(
...
void FootprintListModel::listObjectRemoved(
...
void FootprintListModel::footprintEdited() noexcept {
  if (!mFootprintList) {
    return;
  }
  // int row = mFootprintList->indexOf(static_cast<Footprint*>(sender()));
  // if (row >= 0) {
