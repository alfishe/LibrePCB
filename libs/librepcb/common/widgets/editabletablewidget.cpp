  ...
  if (!indexWidget(index)) {
    ...
    if (row < model()->rowCount() - 1) {
      ...
      if (mShowMoveButtons) {
        ...
        layout->addWidget(
            createButton(QIcon(":/img/actions/down.png"), tr("Move down"), size,
                         size, data, &EditableTableWidget::btnMoveDownClicked));
      }
...
QToolButton* EditableTableWidget::createButton(const QIcon&   icon,
